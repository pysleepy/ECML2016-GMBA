...
def test(data_set_name, mlc, test_name, need_shuffling=False, need_sampling=False, need_normalize=True,
    ...
    # if RAM overflow when running wrapper model, tray main2 while it may be slower.
    ...
    if algorithm_list_filter == [] and algorithm_list_wrapper == []:
        algorithm_list_filter = ['MLFS', 'MLRF', 'SPEC', 'GMBA', 'all_features']
    ...
    for algorithm in algorithm_list_wrapper:
        if mlc != 'CLR':
            hamming_loss[algorithm + '_wrapper'], f1_macro[algorithm + '_wrapper'], f1_micro[algorithm + '_wrapper'],\
             train_time[algorithm + '_wrapper'], test_time[algorithm + '_wrapper'] = wrapper.cross_validation_wrapper(
                    data_name=data_set_name, mlc_type=mlc, compare_with=algorithm, feature_threshold=0,
                    need_normalize=need_normalize, need_sampling=need_sampling, need_contaminate=need_contaminate,
                    need_shuffling=need_shuffling)
            ...
            ranking_loss[algorithm + '_wrapper'], average_precision[algorithm + '_wrapper'], \
             train_time[algorithm + '_wrapper'], test_time[algorithm + '_wrapper'] =\
...
def load_results(data_set_name, mlc, test_name, algorithm_list_filter=[], algorithm_list_wrapper=[]):
    ...
    # algorithm_list_filter = ['GMBA_tuned']
...
if __name__ == '__main__':
    ...
    need_shuffling = False
    # algorithm_list_filter = ['GMBA']
    algorithm_list_filter = ['MLFS', 'MLRF', 'SPEC', 'GMBA', 'all_features']
